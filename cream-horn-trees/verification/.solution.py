...
def create_point_dataset(df):
    ...
    for image_path in df["image_path"].unique():
        ...
        images.append(
            torchvision.transforms.functional.pil_to_tensor(img).float() / 255
        )
...
def augment_image_and_points(image, points, augmentations):
    ...
    for augmentation in augmentations:
        augmented_image, augmented_points = augmentation(
            augmented_image, augmented_points
        )
...
def random_brightness_contrast(
    image, points, brightness_factor_range=(0.6, 1.6), contrast_factor_range=(0.6, 1.6)
):
    """
    ...
    brightness_factor = random.uniform(
        brightness_factor_range[0], brightness_factor_range[1]
    )
...
def random_gaussian_noise(image, points, mean=0, std_dev_range=(0, 0.1)):
    ...
    augmented_image = torch.clamp(
        image + noise, 0, 1
    )  # clamp and convert to uint8 (important)
    return augmented_image, points
...
def random_shear(image, points, shear_factor_range=(-0.2, 0.2)):
    ...
    shear_matrix_np = np.array([[1, shear_factor, 0], [0, 1, 0], [0, 0, 1]])
    shear_matrix_inv_np = np.linalg.inv(shear_matrix_np)
    shear_matrix_inv = [
        shear_matrix_inv_np[0, 0],
        shear_matrix_inv_np[0, 1],
        shear_matrix_inv_np[0, 2],
        shear_matrix_inv_np[1, 0],
        shear_matrix_inv_np[1, 1],
        shear_matrix_inv_np[1, 2],
    ]
...
def random_crop(image, points, crop_factor=(0.6, 1.0)):
    ...
    valid_indices = (
        (augmented_points[:, 0] >= 0)
        & (augmented_points[:, 0] < crop_width)
        & (augmented_points[:, 1] >= 0)
        & (augmented_points[:, 1] < crop_height)
    )
...
if __name__ == "__main__":
    ...
    for i in range(num_augmentations):
        ...
        selected_augmentations = random.sample(
            augmentations, random.randint(1, len(augmentations))
        )
        augmented_image, augmented_points = augment_image_and_points(
            image, points, augmentations
        )
...
def collate_fn(batch):
    ...
    for i in batch:
        augmented_image, augmented_points = augment_image_and_points(
            i["image"].to(device), i["points"], augmentations
        )

        images.append(augmented_image)
        points.append(augmented_points)
        target.append(PointDetectionDataset.get_target(augmented_points))
    return {"image": torch.stack(images), "points": points, "target": target}
...
valid_dataloader = DataLoader(
    valid_ds, batch_size=16, shuffle=False, collate_fn=collate_no_aug
)
...
for epoch in range(150):
    ...
    for batch in tqdm(dataloader):
        ...
        targets = [
            {key: value.to(device) for key, value in t.items()} for t in batch["target"]
        ]
...
def process_segment(
    image_tensor, x1, y1, x2, y2, segment_width=16, output_size=(16, 128)
):
    """
...
class EdgeDataset(Dataset):
    def __init__(self, df):
        ...
        for i in self.files:
            ...
            self.edges.append(
                torch.tensor(
                    df[df["image_path"] == i][["x0", "y0", "x1", "y1"]].to_numpy()
                )
            )
            self.labels.append(
                torch.tensor(
                    [TYPE_INDEX_MAP[x] + 1 for x in df[df["image_path"] == i]["type"]]
                )
            )
    ...
    def get_images(item, std=0.0, bg=0.0):
        ...
        for i in range(n_bg):
            ...
            e = torch.tensor(
                [
                    item["edges"][i1, b1 * 2],
                    item["edges"][i1, b1 * 2 + 1],
                    item["edges"][i2, b2 * 2],
                    item["edges"][i2, b2 * 2 + 1],
                ]
            )
            e2 = torch.tensor(
                [
            ...
            while (
                min(
                    torch.sum((item["edges"] - e) ** 2, axis=1).min(),
                    torch.sum((item["edges"] - e2) ** 2, axis=1).min(),
                )
                < 500
                or ((e[0] - e[2]) ** 2 + (e[1] - e[3]) ** 2) < 50
            ):
                i1 = random.randint(0, len(item["edges"]) - 1)
                i2 = random.randint(0, len(item["edges"]) - 1)
                while i1 == i2:
        ...
        return torch.stack(o), torch.cat(
            (item["labels"], torch.tensor([0 for i in range(n_bg)]))
        )
...
class EdgeClassifier(nn.Module):
    def __init__(self, num_classes=5):
        ...
        self.fc_input_size = (
            128 * 2 * 16
        )  # Calculated based on the output of the convolutional layers
    ...
    def forward(self, x):
        ...
        x = self.fc2(
            x
        )  # No activation on the final layer for classification (use CrossEntropyLoss)
        return x
    ...
    transforms.Compose(
        [
            ...
            transforms.ColorJitter(
                brightness=0.15, contrast=0.15, saturation=0.03, hue=0.005
            ),
        ]
    ),
)
...
for epoch in range(40):
    ...
    with torch.no_grad():
        for images, labels in tqdm(valid_edge_dataloader):
            ...
            correct_valid_acum += (
                (out.argmax(axis=1) == labels.to(device).long()).sum().item()
            )
    model.train()
    print(
        epoch,
        loss_acum / samples_acum,
        loss_valid_acum / samples_valid_acum,
        correct_valid_acum / samples_valid_acum,
    )
...
def your_solution(images_path):
    ...
    for img_path in tqdm(images_path):
        ...
        # return
        tree = dfs(start)
        predictions[img_path] = {"detections": detections, "tree": ("root", tree)}
